...
class BPEVocab:
    def __init__(self, fname_tokenizer: Path) -> None:
        ...
        vocab_size: int = len(self.tokenizer.get_vocab().keys())
        self.added_tokens_list = ["<|startofpiece|>", "<|endofpiece|>", "<|LDWANG|>", "[MASK]", "[gMASK]", "[sMASK]", "[CLS]", "</s>"]
        self.vocab_size_base: int = vocab_size
        self.vocab_size: int = self.vocab_size_base + len(self.added_tokens_list)
...
def load_vocab_aquila(path: Path) -> BPEVocab:
    # Be extra-friendly and accept either a file or a directory.  Also, if it's
    # a directory, it might be the model directory, and tokenizer.model might
    # be in the parent of that.
...
def main(args_in: Optional[List[str]] = None) -> None:
    ...
    if args.dump_single:
        ...
        if model_plus.vocab is not None and args.vocab_dir is None:
            ...
            vocab = load_vocab_aquila(vocab_dir)
